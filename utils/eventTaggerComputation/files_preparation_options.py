...
binning_input_features = {
    ...
    "DeltaPhiMinGoodJetsAK8": np.linspace(0,3.2,41),
    "HT": np.linspace(0,3000,61),
    "ST": np.linspace(0,5000,61)
}
...
def add_variables(ak_array):
    ...
    for variable_name in ["deltaPhiMET", "LundJetPlaneZ", "MTMETLog", "mass"]:
        for idx in range(n_jets):
            ak_array[f"GoodJetsAK8_{variable_name}"] = ak_array[f"JetsAK8_{variable_name}"]
