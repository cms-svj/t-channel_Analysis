...
specialComment = "_egamma" # can add special comment such as _iter2, meaning 2nd iteration, to distinguish from plots from the first iteration, otherwise just leave it as an empty string
cut = "_nocut" # this cut (if other than _pre) shows the effect of the phi spike filter for the particular nSigma
etaMin = -2.4
etaMax = 2.4
phiMin = -3.2
phiMax = 3.2
...
inputFileDict = {
                    # "2016": "/srv/skim_phiSpike_restudy_fixedTrgBug_pre_psFilterSig4_clean/",
                    # "2017": "/srv/skim_phiSpike_restudy_fixedTrgBug_pre_psFilterSig4_clean/",
                    # "2018": "/srv/TM_phiSpike_opt_removeHighWeightQCD_clean/",
                    "2018PreHEM": "/srv/skim_phiSpike_egamma_with_METCut/",    # the QCD file used here is actually for all 2018, but shouldn't change the outcome
                    "2018PostHEM": "/srv/skim_phiSpike_egamma_with_METCut/",  # the QCD file used here is actually for all 2018, but shouldn't change the outcome
}
samples = ["Data"]
# samples = ["Data","QCD"]
variables2D = {
...
# nSigmas = np.arange(3,5,0.5)
nSigmas = [4]
for nSigma in nSigmas:
    nSigmaLab = str(np.round(nSigma,2)).replace(".","p")
    if qcdGt300:
        plotFolderPre = f"plots/{cut[1:]}{specialComment}/sigma{nSigmaLab}/QCD_gt300"
    else:
        plotFolderPre = f"plots/{cut[1:]}{specialComment}/sigma{nSigmaLab}/QCD_all"
