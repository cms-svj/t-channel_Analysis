...
def getHist(self, hist_name):
    """
    Return a ROOT TH1 histogram from this DataSetInfo object.
    Assumes self has attributes: basedir, fileName
    """
    import ROOT
    file_path = os.path.join(self.basedir, self.fileName)
    ...
    # Clone the histogram so file can be closed safely
    hist = hist.Clone()
    f.Close()
    return hist

# Attach to DataSetInfo
info.DataSetInfo.getHist = getHist
...
def compute_s_over_sqrtb(obs_sig, pred_bg):
    """
    obs_sig : list or array of observed signal yields in region A (S)
    pred_bg : list or array of predicted background yields in region A (B) -- from ABCD prediction
    Returns array of S/sqrt(B) with same length as inputs. B<=0 -> np.nan
    """
    S = np.array(obs_sig, dtype=float)
    B = np.array(pred_bg, dtype=float)

    # avoid negative/zero backgrounds
    with np.errstate(divide='ignore', invalid='ignore'):
        denom = np.sqrt(B)
        s_over_sqrtb = np.where(denom > 0, S / denom, np.nan)

    return s_over_sqrtb

def plot_s_over_sqrtb_histogram(zvals, x_edges, y_edges, vmax, title, filename, cmap=None):
    """
    zvals : 1D array of length (len(x_edges)*len(y_edges)) arranged the same way your compute loop produced.
    x_edges : bin edges for MET axis (1D array of length Nx)
    y_edges : bin edges for DNN axis (1D array of length Ny)
    vmax : maximum value to show on color scale (values above clipped)
    """
    plt.figure(figsize=(10, 8))

    # Mesh: note numpy.meshgrid used previously in your script; keep same orientation
    X, Y = np.meshgrid(x_edges, y_edges)

    # Reshape z into (len(x_edges), len(y_edges)) to match previous usage
    try:
        Z = np.array(zvals, dtype=float).reshape(len(x_edges), len(y_edges))
    except Exception as e:
        # fallback if ordering is different: try transpose shape
        Z = np.array(zvals, dtype=float).reshape(len(y_edges), len(x_edges)).T
    ...
    pcm = plt.pcolormesh(X, Y, Z, cmap=cmap, vmin=0, vmax=vmax, shading='auto')
    ...
    # annotate a note for bins with B<=0 (if any)
    n_bad = np.count_nonzero(np.isnan(Z))
    if n_bad > 0:
        plt.annotate(f"{n_bad} empty/invalid bins (B<=0 or NaN)", xy=(0.02, 0.02), xycoords='figure fraction', fontsize=8, color='gray')
...
def plot_1D_hist_fromROOT(Data, sgData, bgData, hist_MET_name, hist_DNN_name, output_dir, svj_label, normalize=False,plot_s_over_sqrtb=True):
    """
    Plot MET and DNN histograms from ROOT files for Data, Signal, and Background.
    bgData and sgData can be lists of DataSetInfo objects — histograms will be summed.
    """
    os.makedirs(output_dir, exist_ok=True)
    
    def sum_histograms(datasets, hist_name):
        """Sum histograms from multiple DataSetInfo objects"""
        total = None
        for ds in datasets:
            h = ds.getHist(hist_name)
            if total is None:
                total = h.Clone(f"sum_{hist_name}")
            else:
                total.Add(h)
        nbins = total.GetNbinsX()
        bin_centers = np.array([total.GetBinCenter(i) for i in range(1, nbins + 1)])
        contents = np.array([total.GetBinContent(i) for i in range(1, nbins + 1)])
        errors = np.array([total.GetBinError(i) for i in range(1, nbins + 1)])
        return bin_centers, contents, errors

    # -------------------- MET --------------------
    #data_x, data_y, _ = sum_histograms([Data], hist_MET_name)
    sig_x, sig_y, _   = sum_histograms([sgData] if not isinstance(sgData, list) else sgData, hist_MET_name)
    bkg_x, bkg_y, _   = sum_histograms([bgData] if not isinstance(bgData, list) else bgData, hist_MET_name)

    if normalize:
        #data_y /= np.sum(data_y) if np.sum(data_y) > 0 else 1
        sig_y  /= np.sum(sig_y)  if np.sum(sig_y)  > 0 else 1
        bkg_y  /= np.sum(bkg_y)  if np.sum(bkg_y)  > 0 else 1

    plt.figure(figsize=(10, 8))
    #plt.step(data_x, data_y, where='mid', color='blue', label='Data', linewidth=1.8)
    plt.step(sig_x,  sig_y,  where='mid', color='red',  label='Signal', linewidth=1.8)
    plt.step(bkg_x,  bkg_y,  where='mid', color='black',label='Background', linewidth=1.8)
    plt.xlabel("MET [GeV]", fontsize=14)
    plt.ylabel("Normalized Counts" if normalize else "Counts", fontsize=14)
    plt.title(f"{svj_label}: MET Distribution", fontsize=15, fontweight='bold')
    plt.xlim(200,1000)
    plt.legend(fontsize=12)
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{svj_label}_MET_overlay.pdf"))
    plt.close()

    

    # -------------------- DNN --------------------
    #data_x, data_y, _ = sum_histograms([Data], hist_DNN_name)
    sig_x, sig_y, _   = sum_histograms([sgData] if not isinstance(sgData, list) else sgData, hist_DNN_name)
    bkg_x, bkg_y, _   = sum_histograms([bgData] if not isinstance(bgData, list) else bgData, hist_DNN_name)
    ...
    plt.ylabel("Normalized Counts" if normalize else "Counts", fontsize=14)
    plt.title(f"{svj_label}: DNN Distribution", fontsize=15, fontweight='bold')
    plt.legend(fontsize=12)
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{svj_label}_DNN_overlay.pdf"))
    plt.close()

    # Plot S/sqrt(B) for DNN
    if plot_s_over_sqrtb:
        s_over_sqrtb = np.zeros_like(sig_y)
        mask = bkg_y > 0
        s_over_sqrtb[mask] = sig_y[mask] / np.sqrt(bkg_y[mask])
        plt.figure(figsize=(10, 8))
        plt.step(sig_x, s_over_sqrtb, where='mid', color='green', label='S/sqrt(B)', linewidth=1.8)
        plt.xlabel("DNN Score", fontsize=14)
        plt.ylabel("S/√B", fontsize=14)
        plt.title(f"{svj_label}: DNN S/√B", fontsize=15, fontweight='bold')
        plt.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"{svj_label}_DNN_SoverSqrtB.pdf"))
        plt.close()

    print(f"✅ Saved MET and DNN overlay plots for {svj_label} → {output_dir}")
...
def main():
    ...
    parser.add_option('-y', dest='year', type='string', default='2018', help="Run year")
    parser.add_option('-o', '--outputdir', dest='outputdir', type='string', default='Hyperparamtrained_NC',
                      help="Top-level output folder name")
    ...
    # =============================
    # Define binning for MET & DNN
    # =============================
    met_bins = 25
    dnn_bins = 25

    MET_inner_edges = np.linspace(205, 500, met_bins)
    ...
    # ---- PNET 2D Non-Closure ----
    # =============================
    output_dir_pnet = f'{base_output}/PNET/2D-Signal_significance/{year}/ControlRegion'
    os.makedirs(output_dir_pnet, exist_ok=True)

    (
        outer_edge_results,
        obs_0_data, pred_0_data, obs_0_sig, pred_0_sig, obs_0_bg, pred_0_bg,
        obs_1_data, pred_1_data, obs_1_sig, pred_1_sig, obs_1_bg, pred_1_bg,
        obs_2_data, pred_2_data, obs_2_sig, pred_2_sig, obs_2_bg, pred_2_bg,
        obs_3_data, pred_3_data, obs_3_sig, pred_3_sig, obs_3_bg, pred_3_bg,
        _, _, _, _, _, _,
        _, _, _, _, _, _,
        _, _, _, _, _, _,
        _, _, _, _, _, _
    ) = compute_ABCD_prediction(
    Data, sgData, bgData, "h_METvsDNN", '_pre_', 
    DNN_inner_edges, DNN_outer_edges, MET_inner_edges, MET_outer_edges
    )
 
    # compute S/sqrt(B) where S = signal observed A (obs_*_sig) and B = predicted background A (pred_*_bg)
    s_over_sqrtb_0 = compute_s_over_sqrtb(obs_0_sig, obs_0_bg)
    s_over_sqrtb_1 = compute_s_over_sqrtb(obs_1_sig, obs_1_bg)
    s_over_sqrtb_2 = compute_s_over_sqrtb(obs_2_sig, obs_2_bg)
    s_over_sqrtb_3 = compute_s_over_sqrtb(obs_3_sig, obs_3_bg)

    MET_edges_plot = np.linspace(200, 500, met_bins)
    DNN_edges_plot = np.linspace(0, 1, dnn_bins)

    vmax = np.nanpercentile(np.hstack([s_over_sqrtb_0, s_over_sqrtb_1, s_over_sqrtb_2, s_over_sqrtb_3]), 98)
    # avoid zero vmax
    if not np.isfinite(vmax) or vmax <= 0:
        vmax = 5.0

    plot_s_over_sqrtb_histogram(s_over_sqrtb_0, MET_edges_plot, DNN_edges_plot, vmax, '0SVJ S/sqrt(B)', f"{output_dir_pnet}/SoverSqrtB_0SVJ.pdf")
    plot_s_over_sqrtb_histogram(s_over_sqrtb_1, MET_edges_plot, DNN_edges_plot, vmax, '1SVJ S/sqrt(B)', f"{output_dir_pnet}/SoverSqrtB_1SVJ.pdf")
    plot_s_over_sqrtb_histogram(s_over_sqrtb_2, MET_edges_plot, DNN_edges_plot, vmax, '2SVJ S/sqrt(B)', f"{output_dir_pnet}/SoverSqrtB_2SVJ.pdf")
    plot_s_over_sqrtb_histogram(s_over_sqrtb_3, MET_edges_plot, DNN_edges_plot, vmax, '3PSVJ S/sqrt(B)', f"{output_dir_pnet}/SoverSqrtB_3PSVJ.pdf")


    print(f"PNET 2D S over root B  saved to: {output_dir_pnet}")

    print(f"PNET 2D S/√B plots saved to: {output_dir_pnet}")

    # ===========================================================
    # ---- 1D ROOT-based Signal vs Background plots for all SVJ ----
    # ===========================================================
    for svj in ['0SVJ', '1SVJ', '2SVJ', '3PSVJ']:
        plot_1D_hist_fromROOT(Data=Data[0],sgData=sgData,bgData=bgData,hist_MET_name=f"h_MET_pre_{svj}",hist_DNN_name=f"h_dnnEventClassScore_pre_{svj}",output_dir=output_dir_pnet,svj_label=svj,normalize=False,plot_s_over_sqrtb=True)

    # =============================
    # ---- WNAE 2D Non-Closure ----
    # =============================
    output_dir_wnae = f'{base_output}/WNAE/2D-Signal_significance/{year}/ControlRegion'
    ...
    (
        ...
        Data, sgData, bgData, "h_METvsDNN", '_pre_WNAE_',
        DNN_inner_edges, DNN_outer_edges, MET_inner_edges, MET_outer_edges
    )
    ...
    plot_s_over_sqrtb_histogram(s_over_sqrtb_0, MET_edges_plot, DNN_edges_plot, vmax, '0SVJ S/sqrt(B)', f"{output_dir_wnae}/SoverSqrtB_0SVJ.pdf")
    plot_s_over_sqrtb_histogram(s_over_sqrtb_1, MET_edges_plot, DNN_edges_plot, vmax, '1SVJ S/sqrt(B)', f"{output_dir_wnae}/SoverSqrtB_1SVJ.pdf")
    plot_s_over_sqrtb_histogram(s_over_sqrtb_2, MET_edges_plot, DNN_edges_plot, vmax, '2SVJ S/sqrt(B)', f"{output_dir_wnae}/SoverSqrtB_2SVJ.pdf")
    plot_s_over_sqrtb_histogram(s_over_sqrtb_3, MET_edges_plot, DNN_edges_plot, vmax, '3PSVJ S/sqrt(B)', f"{output_dir_wnae}/SoverSqrtB_3PSVJ.pdf")

    print(f"WNAE 2D S over root B saved to: {output_dir_wnae}")
    for svj in ['0SVJ', '1SVJ', '2SVJ', '3PSVJ']:
        plot_1D_hist_fromROOT(Data=Data[0],sgData=sgData,bgData=bgData,hist_MET_name=f"h_MET_pre_WNAE_{svj}",hist_DNN_name=f"h_dnnEventClassScore_pre_WNAE_{svj}",output_dir=output_dir_wnae,svj_label=svj,normalize=False,plot_s_over_sqrtb=True)
