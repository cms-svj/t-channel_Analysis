...
def plotStack(data, histoName, totalBin, outputPath="./", xTitle="", yTitle="", isLogY=False, xmin=999.9, xmax=-999.9, norm=False, normBkg=False, onlySig=False, stList=None, yieldValues=None, year="2017", isRatio=False, hemPeriod=False, showEvent = False,ispdf=False,isN1plots = False , MakeLineAtBin=None):
    ...
    if(data[2]):
        #firstPass=True
        for d in data[2]:
            h = d.getHisto(histoName, rebinx=rebinx, xmin=xmin, xmax=xmax, showEvents=showEvent)
            if (stList != None) and (not normBkg):
                newEntry = stList + [getLabel(d.legEntry()),round(h.Integral())]
                yieldValues.loc[len(yieldValues.index)] = newEntry
    ...
    if(data[0]):
        for d in data[0]:
            datahist = d.getHisto(histoName, rebinx=rebinx,xmin=xmin,xmax=xmax,showEvents=showEvent)
            if (stList != None) and (not normBkg):
