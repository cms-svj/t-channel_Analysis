...
def GetABCDhistPerSVJBin(data, ABCDhistoVar, maincut,WP, SVJbin,DNN_inner_edge,DNN_outer_edge,MET_inner_edge,MET_outer_edge):
    ...
    SVJ, (dnn, met) = SVJbin
    histName = ABCDhistoVar + maincut + SVJ + WP
...
def get_ABCD_counts_by_SVJ(Data,sgData, bgData, ABCDHistoVar, maincut,WP,SVJBins,DNN_inner_edge,DNN_outer_edge,MET_inner_edge,MET_outer_edge):
    """
    Extracts the ABCD region counts split by the number of SVJ jets for both signal and background.
    """
    counts_by_SVJ = {}
    error_by_SVJ = {}
    for data_list, label in [(bgData, "Background")]:
        for data in data_list:  # Iterate over individual dataset objects
            for svj_bin in SVJBins.items():
                ...
                hist_dict = GetABCDhistPerSVJBin(data, ABCDHistoVar, maincut,WP, svj_bin,DNN_inner_edge,DNN_outer_edge,MET_inner_edge,MET_outer_edge)
...
def compute_ABCD_prediction(Data, sgData, bgData, ABCDHistoVar, maincut,WP, DNN_inner_edges, DNN_outer_edges, MET_inner_edges, MET_outer_edges):
    outer_edge_results = {}  
    ...
    for (DNN_inner_edge, DNN_outer_edge), (MET_inner_edge, MET_outer_edge) in product(zip(DNN_inner_edges, DNN_outer_edges),zip(MET_inner_edges, MET_outer_edges)):
        ...
        counts_by_SVJ, error_by_SVJ = get_ABCD_counts_by_SVJ(
            Data, sgData, bgData, ABCDHistoVar, maincut,WP, SVJBins, 
...
def main():
    ...
    output_dir = f'Nonclosure/2D-NonC/allbckg'
    
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    WorkingPoints = ['_wpt0p70','_wpt0p72','_wpt0p75','_wpt0p80','_wpt0p85','_wpt0p87','_wpt0p90' ]
    for WP in WorkingPoints:
        ...
        obserr_2PSVJ_data, obserr_2PSVJ_sig, obserr_2PSVJ_bg, prederr_2PSVJ_data, prederr_2PSVJ_sig, prederr_2PSVJ_bg, \
        obserr_3PSVJ_data, obserr_3PSVJ_sig, obserr_3PSVJ_bg, prederr_3PSVJ_data, prederr_3PSVJ_sig, prederr_3PSVJ_bg = \
            compute_ABCD_prediction(Data, sgData, bgData, "h_METvsDNN", '_pre_',WP,DNN_inner_edges, DNN_outer_edges, MET_inner_edges, MET_outer_edges)
        ...
        plot_nonclosure_histogram(nonclosure_2PSVJ, MET_edges,DNN_edges,1, f'2PSVJ {WP}', f"{output_dir}/nonclosure_2PSVJ_{WP}.png")
        plot_nonclosure_histogram(nonclosure_3PSVJ, MET_edges,DNN_edges,1, f'3PSVJ {WP}', f"{output_dir}/nonclosure_3PSVJ_{WP}.png")
